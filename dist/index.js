...
async function uploadFiles(shasumsUpload, shasumsSigUpload, outputDir, repoName, tag) {
...
async function run() {
    try {
        ...
        const osArchPairs = core.getInput('osArch', { required: true })
            .split(',')
            .map(pair => pair.trim());
        const result = await getAssetDownloadUrls(githubRepo, authToken, osArchPairs);
