...
/* harmony import */ var _octokit_request__WEBPACK_IMPORTED_MODULE_3__ = __nccwpck_require__(234);
/* harmony import */ var _octokit_request__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__nccwpck_require__.n(_octokit_request__WEBPACK_IMPORTED_MODULE_3__);




const pipeline = (0,util__WEBPACK_IMPORTED_MODULE_2__.promisify)((__nccwpck_require__(781).pipeline));
async function downloadFile(url, outputFilePath, authToken) {
    const response = await (0,_octokit_request__WEBPACK_IMPORTED_MODULE_3__.request)("GET " + url, {
        headers: {
            Accept: "application/octet-stream",
            Authorization: `token ${authToken}`
        }
    });
    const file = fs__WEBPACK_IMPORTED_MODULE_1__.createWriteStream(outputFilePath);
    response.data.pipe(file);
    file.on('finish', () => {
        file.close();
    });
    file.on('error', (err) => {
        fs__WEBPACK_IMPORTED_MODULE_1__.unlinkSync(outputFilePath);
        throw err.message;
    });
}
async function getAssetUrl(releasesUrl, authToken) {
    const response = await (0,_octokit_request__WEBPACK_IMPORTED_MODULE_3__.request)("GET " + releasesUrl, {
...
async function run() {
    try {
        const releasesUrl = _actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput('releases-url', { required: true });
        const outputFilePath = _actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput('output-file-path', { required: true });
        const authToken = _actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput('auth-token', { required: false });
        const assetUrl = await getAssetUrl(releasesUrl, authToken);
        await downloadFile(assetUrl, outputFilePath, authToken);
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.setOutput('downloaded-file-path', outputFilePath);
    }
    catch (error) {
