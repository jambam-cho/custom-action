...
var dist_node = __nccwpck_require__(6234);
...
const src_pipeline = (0,external_util_.promisify)((__nccwpck_require__(2781).pipeline));
async function downloadFile(url, outputFilePath, authToken) {
    ...
    if (response.status === 200) {
        ...
        file.on('error', (err) => {
            external_fs_.unlinkSync(outputFilePath);
            throw err.message;
        });
    }
    else {
...
async function getAssetUrl(releasesUrl, authToken) {
    const response = await (0,dist_node.request)("GET " + releasesUrl, {
